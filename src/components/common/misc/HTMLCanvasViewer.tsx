...
interface HTMLCanvasViewerProps {
  /** HTML string content to render and convert to canvas */
  htmlString: string;
}

/**
 * Ref interface for HTMLCanvasViewer component
 */
export interface HTMLCanvasViewerRef {
  /** Method to download the rendered content as PDF */
...
/**
 * A component that renders HTML content to a canvas and provides PDF export functionality.
 * Uses html2canvas to convert HTML to image and jsPDF to generate PDFs.
 *
 * @example
 * ```tsx
 * const viewerRef = useRef<HTMLCanvasViewerRef>(null);
 *
 * return (
 *   <HTMLCanvasViewer
 *     ref={viewerRef}
 *     htmlString="<h1>Hello World</h1><p>Some content</p>"
 *   />
 * );
 *
 * // Download as PDF
 * viewerRef.current?.downloadPDF('document');
 * ```
 */
