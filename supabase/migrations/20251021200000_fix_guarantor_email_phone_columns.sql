-- Fix RPC function: Remove references to non-existent guar.email and guar.phone columns
-- The guarantors table doesn't have email and phone columns, so set them to NULL
        ...
        COALESCE(
            (
                SELECT jsonb_agg(jsonb_build_object(
                    ...
                    'guarantor_email', NULL,  -- Fixed: guarantors table doesn't have email column
                    'guarantor_phone', NULL,  -- Fixed: guarantors table doesn't have phone column
...
'Obtiene todas las propiedades de un usuario con el conteo de postulaciones y detalles completos de cada postulación incluyendo datos del postulante y aval. Versión corregida: guarantor_email y guarantor_phone se establecen en NULL ya que la tabla guarantors no tiene estas columnas.';
